...
export class UploadService {
    ...
    async generatePresignedUrl(options: PresignedUrlOptions): Promise<PresignedUrlResult> {
        const { fileName, contentType, expiresInMinutes = 60 } = options;
        ...
        try {
            ...
            return { uploadUrl, blobUrl, expiresIn: expiresInMinutes * 60 };
        } catch (error) {
    ...
    async uploadFile(containerName: string, fileName: string, fileBuffer: Buffer, contentType: string): Promise<string> {
        try {
            const key = fileName;
    ...
    private async ensureContainerExists(containerName: string): Promise<void> {
        try {
            const command = new HeadBucketCommand({ Bucket: containerName });
            await this.s3Client.send(command);
    ...
    generateDocumentBlobPath(resortSlug: string, docType: string, fileName: string): string {
        const extension = this.getFileExtensionFromName(fileName);
        const uuid = randomUUID();
        const timestamp = new Date().toISOString().slice(0, 10);
